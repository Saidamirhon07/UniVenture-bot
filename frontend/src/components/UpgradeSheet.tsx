...
export default function UpgradeSheet({ feature, subscription, onClose, onUnlocked }: { feature: string; subscription: SubscriptionStatus; onClose: () => void; onUnlocked: (next: SubscriptionStatus) => void }) {
  ...
  return <div className="upgrade-backdrop" role="presentation" onClick={onClose}>
    <section className="upgrade-sheet" role="dialog" aria-modal="true" aria-labelledby="upgrade-title" onClick={(event) => event.stopPropagation()}>
      ...
      <label className="upgrade-consent"><input type="checkbox" checked={accepted} onChange={(event) => setAccepted(event.target.checked)} /><i>{accepted ? <Check size={13} /> : null}</i><span><strong>I agree to the payment terms</strong><small>Admin-verified payment for 30 days of Premium access.</small></span></label>
      <button className="upgrade-pay" disabled={!accepted || loading || !subscription.card_number} onClick={() => void startPayment()}>{loading ? <LoaderCircle className="spin" size={18} /> : <Send size={18} />}I paid — ask me for the receipt</button>
